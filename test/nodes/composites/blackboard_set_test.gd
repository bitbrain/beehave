...
func test_set_to_constant() -> void:
	...
	assert_that(blackboard_set.tick(actor, blackboard, delta)).is_equal(BeehaveNode.SUCCESS)
...
func test_invalid_expression() -> void:
	...
	var runner: GdUnitSceneRunner = scene_runner(blackboard_set)
	assert_that(blackboard_set.tick(actor, blackboard, delta)).is_equal(BeehaveNode.FAILURE)
