...
func test_comparison_operators() -> void:
	...
	for operands in data:
		for pair in data[operands]:
			...
			assert_that(blackboard_compare.tick(actor, blackboard, delta)).is_equal(expected_status)
...
func test_blackboard_access() -> void:
	...
	runner = scene_runner(blackboard_compare)
	assert_that(blackboard_compare.tick(actor, blackboard, delta)).is_equal(BeehaveNode.SUCCESS)
...
func test_invalid_left_operand_expression() -> void:
	...
	runner = scene_runner(blackboard_compare)
	assert_that(blackboard_compare.tick(actor, blackboard, delta)).is_equal(BeehaveNode.FAILURE)
